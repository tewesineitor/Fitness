...
export const PotentiationScreen: React.FC<PotentiationScreenProps> = ({ exerciseName, onStartWorkout }) => (
  <div className="flex h-full flex-col">
    {/* Atmospheric glow */}
    <div className="pointer-events-none absolute inset-x-0 top-0 h-72 bg-[radial-gradient(ellipse_80%_60%_at_50%_0%,rgba(var(--color-brand-accent-rgb),0.15),transparent_70%)]" />
    ...
    <div className="relative z-10 flex flex-1 flex-col items-center justify-center gap-8 px-6 text-center">
      ...
      <div className="flex flex-col gap-3">
        ...
        <p className="text-xl font-black text-brand-accent leading-tight">{exerciseName}</p>
      </div>
    ...
    {/* Fade mask */}
    <div className="pointer-events-none absolute bottom-0 left-0 w-full h-40 bg-gradient-to-t from-bg-base to-transparent z-10" />

    {/* FAB */}
    <div className="relative z-20 flex flex-col items-center px-6 pb-10 pt-4">
      <button
        onPointerDown={() => { vibrate(15); onStartWorkout(); }}
        className="bg-brand-accent text-brand-accent-foreground font-black text-lg rounded-full py-5 w-full max-w-sm mx-auto active:scale-[0.97] transition-all select-none"
        style={{ letterSpacing: 'var(--letter-spacing-caps)' }}
      >
        Comenzar
      </button>
    ...
    <div className="flex h-full flex-col">
      ...
      <div className="relative z-10 flex flex-col gap-3 px-6 pt-4">
        ...
        <div className="flex items-center gap-1.5">
          {Array.from({ length: totalItems }).map((_, index) => (
            <div key={index} className="h-1.5 flex-1 rounded-full overflow-hidden bg-surface-raised">
      ...
      <div className="relative z-10 flex flex-1 flex-col items-center justify-center gap-8 px-6 text-center">
        ...
        <div className="flex flex-col items-center gap-3">
          <h2 className="font-heading text-4xl font-black text-text-primary leading-tight tracking-tight">
            {currentExercise.name}
          </h2>
          {currentItemReps && (
            <p className="text-3xl font-black text-brand-accent uppercase tracking-widest">
              {currentItemReps}
            </p>
          )}
        </div>
        ...
        <button
          ...
          className="flex items-center gap-1.5 text-xs font-bold text-text-muted hover:text-text-secondary transition-colors select-none"
        >
      ...
      <div className="relative z-20 flex flex-col items-center gap-2 px-6 pb-10 pt-4">
        ...
        <button
          ...
          className="text-text-muted font-bold py-3 text-sm uppercase select-none hover:text-text-secondary transition-colors"
          style={{ letterSpacing: 'var(--letter-spacing-caps)' }}
        >
          Saltar todo
        </button>
